...
export const updatePermissionSchema = Joi.object({
  name: Joi.string().min(3).max(30).error(createError.BadRequest('permission is not correct')),
  description: Joi.string().min(0).max(100).error(createError.BadRequest('description is not correct')),
})
